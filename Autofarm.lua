...
function lib:New(data)
	...
	function library:Dropdown(data)
		...
		data.Value = type(data.Value) == "string"
			and (data.Options[table.find(data.Options, data.Value)] or "click to select")
		...
		local function createDropdown()
			...
			local new = instance("Frame", {
				...
				Position = udim2(
					0,
					newDropdown.AbsolutePosition.X + newDropdown.AbsoluteSize.X + 10,
					0,
					newDropdown.AbsolutePosition.Y + 36
				),
